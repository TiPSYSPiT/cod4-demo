...
The field names match the JSON output (`py source/py/dm1.py DEMO --json`) and are
...
If it is available under `source/maps/<map>.png`, it is used as the floor plan: it
...
An inventory across all eleven demos (`py source/py/inventory.py`) shows what
...
`source/py/verify_all.py` works through three levels:
...
```bash
py source/py/inventory.py demos
```
